        ...
        >
          ...
          <div className="flex justify-between items-center p-6 border-b border-slate-800 bg-slate-900/60">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-white">Digital AI Prescription</h3>
                <p className="text-xs text-slate-400">Patient: <span className="text-white font-medium">{patient.name} ({patient.age}y/o, {patient.gender})</span></p>
              </div>
            </div>
          ...
          <div className="p-6 overflow-y-auto space-y-6 flex-1">
            ...
            <div className="bg-gradient-to-r from-teal-500/10 to-indigo-500/10 border border-teal-500/20 p-4 rounded-xl flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-300 mt-1">Suggest meds based on triage symptoms ({patient.symptoms?.join(', ') || 'Fever & Fatigue'}).</p>
              </div>
              <button 
            ...
            {/* Quick Add Meds */}
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Select Medicines</label>
              <div className="grid grid-cols-2 gap-2">
                {COMMON_MEDICATIONS.map((med, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleAddMed(med)}
                    className="text-left p-2.5 rounded-lg border border-slate-800 bg-slate-900/50 hover:bg-slate-800 hover:border-slate-700 transition-all text-xs"
                  >
            ...
            {/* Selected Rx List */}
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Prescribed Medicines ({selectedMeds.length})</label>
                  ...
                  {selectedMeds.map((med, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-slate-800/60 border border-slate-700/60">
                      <div>
                        ...
                        <span className="text-xs text-slate-400 block">{med.dose} • {med.duration}</span>
                        {med.interaction && (
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1">Doctor Advice / Diet Instructions</label>
              <textarea 
                rows={2} 
                value={notes} 
                onChange={(e) => setNotes(e.target.value)}
                placeholder="Take plenty of fluids, complete antibiotic course..."
                className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-[var(--color-primary)]"
              />
            </div>
          </div>
