    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
        <motion.div
          ...
          className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col"
        >
          {/* Modal Header */}
          <div className="flex justify-between items-center p-5 border-b border-slate-800 bg-slate-900/60">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-white">Digital ABHA Health Card</h3>
                <p className="text-xs text-slate-400">Ayushman Bharat Digital Mission (Govt of India)</p>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-slate-900 via-teal-950 to-indigo-950 border-2 border-teal-500/30 rounded-2xl p-5 shadow-2xl relative overflow-hidden">
              ...
              <div className="flex justify-between items-start border-b border-teal-500/20 pb-3 mb-4">
                <div>
                  ...
                  <h4 className="text-base font-black text-white tracking-wide">ABHA CARD</h4>
                </div>
              ...
              <div className="flex justify-between items-center gap-4">
                <div className="space-y-1.5 flex-1">
                  <div>
                    <span className="text-[9px] text-slate-400 block uppercase font-mono">Patient Name</span>
                    <span className="text-sm font-bold text-white tracking-wide">{patient.name}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <span className="text-[9px] text-slate-400 block uppercase font-mono">Age / Sex</span>
                      <span className="text-xs font-semibold text-slate-200">{patient.age} Yrs / {patient.gender}</span>
                    </div>
                    <div>
                      <span className="text-[9px] text-slate-400 block uppercase font-mono">Blood Group</span>
                      <span className="text-xs font-bold text-rose-400">O +ve</span>
                    </div>
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-400 block uppercase font-mono">ABHA Number</span>
                    <span className="text-xs font-mono font-black text-teal-300 tracking-wider">{abhaNumber}</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-400 block uppercase font-mono">ABHA Address</span>
                    <span className="text-[11px] font-mono text-slate-300">{abhaAddress}</span>
                  </div>
                </div>
              ...
              {/* Card Footer */}
              <div className="mt-4 pt-2 border-t border-slate-800/80 flex justify-between items-center text-[9px] text-slate-400 font-mono">
