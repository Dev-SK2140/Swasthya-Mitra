    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Process test requests and upload results</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {pendingTests.map((test, i) => (
                <tr key={i} className="hover:bg-white/60 dark:bg-slate-800/60 transition-colors group">
                  <td className="p-4 text-sm text-slate-600 dark:text-slate-300 font-mono">{test.id}</td>
                  <td className="p-4">
                    <div className="text-sm font-medium text-slate-900 dark:text-white">{test.patient}</div>
                    <div className="text-xs text-slate-500 dark:text-slate-400">{test.age} yrs • {test.date}</div>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-sm text-slate-600 dark:text-slate-300">{test.doctor}</td>
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${test.priority === 'High' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/20' : 'bg-slate-700 text-slate-600 dark:text-slate-300'}`}>
                      {test.priority}
                    </span>
                  </td>
                  <td className="p-4">
                    <div className="flex gap-2">
                      <button className="flex items-center gap-1 bg-purple-600 hover:bg-purple-500 text-slate-900 dark:text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors shadow">
                        <Upload className="w-3.5 h-3.5" /> Upload & AI Analyze
                      </button>
                      <button className="flex items-center gap-1 bg-slate-700 hover:bg-slate-600 text-slate-700 dark:text-slate-200 px-2.5 py-1.5 rounded-lg text-xs transition-colors" title="Print Sample Barcode">
                        🏷️ Label
                      </button>
                    </div>
                  </td>
